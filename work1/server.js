...
http.createServer(function(req,res){
    ...
    else if(pathName.indexOf('.jpg') >= 0 || pathName.indexOf('.png') >= 0 || pathName.indexOf('.jpeg') >= 0  ){
    ...
    else if(pathName == "/listmanager1"){
        showLM1(res);
    }
    else if(pathName == "/chapterList1"){
        showLM1(res);
    }
...
function showLogin(res){
    var indexPath3 = path.join(__dirname,'login.html');
    var fileContent3 = fs.readFileSync(indexPath3);
    res.writeHead(200,{'Content-Type':'text/html'});
...
function showListManager(res){
    var indexPath4 = path.join(__dirname,'list.html');
    var fileContent4 = fs.readFileSync(indexPath4);
    res.writeHead(200,{'Content-Type':'text/html'});
    res.end(fileContent4);
}
function showAddChapter(res){
    var indexPath5 = path.join(__dirname,'addChapter.html');
    var fileContent5 = fs.readFileSync(indexPath5);
    res.writeHead(200,{'Content-Type':'text/html'});
    res.end(fileContent5);
}

function showImg(res,pathName){
    var ImgPath = path.join(__dirname,pathName);
    var ImgContent = fs.readFileSync(ImgPath);
    if(pathName.indexOf('.jpg')){
        res.writeHead(200,{'Content-Type':'image/jpg'});
    }
    else if(pathName.indexOf('.png')){
        res.writeHead(200,{'Content-Type':'image/png'});
    }
    else if(pathName.indexOf('.jpeg')){
...
function add(req,res){
    ...
    req.on("end",function(){
        if(addStr){
            var file={};
            file.chapterId = chapterList.length+1;
            file.chapterName=addStr;
            file.views = 0;
            List.push(file);
            chapterList.push.apply(chapterList,List);
            List=[];
        }
        var listStr = JSON.stringify(chapterList);
        res.writeHead(200,{"Content-Type":"textml;text/plain;charset:utf-8"});
        res.end(listStr);
    })
};
